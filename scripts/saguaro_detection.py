...
intermediateFolder = arcpy.GetParameterAsText(3)
finishedFolder = arcpy.GetParameterAsText(4)
...
for filename in largefilelist:
    ...
    for diffpoint in torunlist:
        ...
        arcpy.gp.FocalStatistics_sa(filenamec, shadow_mean,
                                    "Irregular {0}\\customk{1}.txt".format(intermediateFolder, diffpoint[1]), "MEAN", "DATA")
        print 'step2 - Summing below shadow pixels'
        # Process: Focal Statistics (3)
        arcpy.gp.FocalStatistics_sa(filenamec, above_mean,
                                    "Irregular {0}\\customkabove{1}.txt".format(intermediateFolder, diffpoint[1]), "MEAN", "DATA")
        print 'step3 - Summing above shadow pixels'
        # Process: Focal Statistics (2)
        arcpy.gp.FocalStatistics_sa(filenamec, below_mean,
                                    "Irregular {0}\\customkbelow{1}.txt".format(intermediateFolder, diffpoint[1]), "MEAN", "DATA")
        ...
        arcpy.gp.FocalStatistics_sa(ct_reclas_4, findSWall_5, "Weight {0}\\s{1}.txt".format(intermediateFolder,diffpoint[1]),
                                    "SUM", "DATA")
        ...
        arcpy.gp.FocalStatistics_sa(scaledpts_7, allones_8,
                                    "Weight {0}\\shiftback{1}.txt".format(intermediateFolder,diffpoint[1]), "SUM", "DATA")
        ...
        if morewvalue_11.mean == None or  morewvalue_11.mean == 0:
            ...
            if addmissed_18.extent.width<=0.5:
                ...
                arcpy.gp.FocalStatistics_sa(addmissed_18, shifted32, "Weight {0}\\shifttoshad{1}.txt".format(intermediateFolder,diffpoint[1]),
                                            "SUM", "DATA")
